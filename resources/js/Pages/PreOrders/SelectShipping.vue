        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-6 shadow-sm">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <!-- Transfer Bank -->
              <label :class="['flex items-start gap-4 p-4 rounded-2xl border cursor-pointer transition-all', form.payment_method === 'transfer' ? 'border-primary bg-primary/[0.02]' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600']">
                <input type="radio" value="transfer" v-model="form.payment_method" class="sr-only">
                <div :class="['w-5 h-5 rounded-full border flex items-center justify-center shrink-0 mt-0.5', form.payment_method === 'transfer' ? 'border-primary' : 'border-gray-300 dark:border-gray-600']">
                  <div v-if="form.payment_method === 'transfer'" class="w-2.5 h-2.5 rounded-full bg-primary"></div>
                </div>
                <div>
                  <h3 class="font-bold text-gray-900 dark:text-white flex items-center gap-1.5 text-sm">
              ...
              <!-- QRIS -->
              <label :class="['flex items-start gap-4 p-4 rounded-2xl border cursor-pointer transition-all', form.payment_method === 'qris' ? 'border-primary bg-primary/[0.02]' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600']">
                <input type="radio" value="qris" v-model="form.payment_method" class="sr-only">
                <div :class="['w-5 h-5 rounded-full border flex items-center justify-center shrink-0 mt-0.5', form.payment_method === 'qris' ? 'border-primary' : 'border-gray-300 dark:border-gray-600']">
            ...
            <div class="mt-6 p-4 bg-gray-50 dark:bg-gray-750 rounded-2xl border border-gray-100 dark:border-gray-700 transition-all">
              <div v-if="form.payment_method === 'transfer'">
                <p class="text-sm font-semibold text-gray-900 dark:text-white mb-2 flex items-center gap-1">
                ...
                <ul class="text-xs text-gray-600 dark:text-gray-400 space-y-2">
                  <li class="p-2.5 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 flex items-center justify-between">
                    <div>
                      <span class="font-bold text-gray-900 dark:text-white">Bank BRI</span>
                      <p class="mt-0.5 font-mono text-gray-500">0123-4567-8901</p>
                    </div>
                    <span class="text-gray-400 text-[10px]">A.N UD Flamboyan</span>
                  </li>
                  <li class="p-2.5 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 flex items-center justify-between">
                    <div>
                      <span class="font-bold text-gray-900 dark:text-white">Bank BNI</span>
                      <p class="mt-0.5 font-mono text-gray-500">9876-5432-1098</p>
                    </div>
                    <span class="text-gray-400 text-[10px]">A.N UD Flamboyan</span>
                  </li>
                </ul>
                <p class="text-[11px] text-gray-400 mt-3">Silakan transfer sesuai nominal total. Bukti transfer di-upload setelah pembayaran dikonfirmasi.</p>
              </div>
              <div v-else-if="form.payment_method === 'qris'" class="flex items-start gap-3 text-sm">
                <Icon icon="mdi:qrcode-scan" class="text-3xl text-primary shrink-0 mt-0.5" />
                <div>
                  <p class="font-semibold text-gray-900 dark:text-white">QRIS Code Barcode</p>
                  <p class="text-xs text-gray-500 mt-1">Barcode QRIS resmi akan ditampilkan di halaman detail pre-order segera setelah Anda menekan tombol "Konfirmasi & Proses".</p>
                </div>
              </div>
            </div>
          </div>
