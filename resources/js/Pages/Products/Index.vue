  ...
  <UserLayout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div v-if="products.data.length" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-6">
        <div v-for="product in products.data" :key="product.id" class="group bg-white rounded-xl sm:rounded-2xl border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 flex flex-col">
          ...
          <div class="p-3 sm:p-4 flex flex-col flex-1">
            <h3 class="font-semibold text-xs sm:text-base text-text mt-1 group-hover:text-primary transition-colors line-clamp-2 leading-snug">{{ product.name }}</h3>
            <p class="text-[10px] sm:text-sm text-gray-500 mt-1">{{ product.weight }}g</p>
