  ...
  <AdminLayout>
    ...
    <div class="grid lg:grid-cols-3 gap-6">
      ...
      <div class="bg-white rounded-2xl border border-gray-100 p-6">
        ...
        <div class="space-y-3">
          <Link v-for="order in recentOrders" :key="order.id" :href="`/admin/orders/${order.id}`" class="block p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition">
          ...
          <p v-if="!recentOrders.length" class="text-sm text-gray-500 text-center py-4">Belum ada pesanan</p>
        </div>
      </div>
    </div>
    ...
    <div class="grid sm:grid-cols-2 gap-4 mt-6">
      <div v-if="stats.pendingPayments > 0" class="bg-yellow-50 border border-yellow-200 rounded-xl p-4 flex items-center gap-3">
        <Icon icon="mdi:clock-alert" class="text-2xl text-yellow-600" />
        <div><p class="font-medium text-yellow-800">{{ stats.pendingPayments }} pembayaran menunggu verifikasi</p><Link href="/admin/orders?status=menunggu_verifikasi" class="text-sm text-yellow-600 hover:underline">Lihat →</Link></div>
      </div>
      <div v-if="stats.lowStockProducts > 0" class="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3">
        <Icon icon="mdi:alert" class="text-2xl text-red-600" />
        <div><p class="font-medium text-red-800">{{ stats.lowStockProducts }} produk stok hampir habis</p><Link href="/admin/stock" class="text-sm text-red-600 hover:underline">Lihat →</Link></div>
      </div>
    </div>
  </AdminLayout>
</template>
...
const statCards = computed(() => [
  { label: 'Total Produk', value: props.stats.totalProducts, icon: 'mdi:package-variant-closed', bgClass: 'bg-blue-50', iconClass: 'text-blue-500' },
  { label: 'Total Pesanan', value: props.stats.totalOrders, icon: 'mdi:cart-check', bgClass: 'bg-green-50', iconClass: 'text-green-500' },
  { label: 'Total Penjualan', value: `Rp ${fmt(props.stats.totalSales)}`, icon: 'mdi:cash-multiple', bgClass: 'bg-orange-50', iconClass: 'text-orange-500' },
  { label: 'Total Pelanggan', value: props.stats.totalUsers, icon: 'mdi:account-group', bgClass: 'bg-purple-50', iconClass: 'text-purple-500' },
]);
...
const chartSeries = computed(() => [{ name: 'Penjualan', data: props.monthlyData }]);
