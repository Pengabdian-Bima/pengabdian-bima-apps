            ...
            <tr v-for="p in products.data" :key="p.id" class="border-t border-gray-50 hover:bg-gray-50/50">
              ...
              <td class="px-4 py-3">
                <div class="flex items-center gap-1">
                  <Link :href="`/admin/products/${p.id}/edit`" class="p-1.5 text-blue-500 bg-blue-100 border border-blue-200 hover:text-blue-700 hover:bg-blue-200 rounded-lg transition"><Icon icon="mdi:pencil" /></Link>
                  <button @click="deleteProduct(p.id)" class="p-1.5 text-red-500 bg-red-100 border border-red-200 hover:text-red-700 hover:bg-red-200 rounded-lg transition"><Icon icon="mdi:trash-can" /></button>
                </div>
              </td>
            </tr>
          </tbody>
