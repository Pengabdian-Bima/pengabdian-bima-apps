            ...
            <tr v-for="p in products.data" :key="p.id" class="border-t border-gray-50 hover:bg-gray-50/50 whitespace-nowrap">
              ...
              <td class="px-4 py-3">
                <div class="flex items-center gap-1">
                  <button @click="openAdjust(p, 'in')" class="p-1.5 text-green-500 bg-green-100 border border-green-200 hover:text-green-700 hover:bg-green-200 rounded-lg transition" title="Tambah Stok"><Icon icon="mdi:plus-circle" /></button>
                  <button @click="openAdjust(p, 'out')" class="p-1.5 text-red-500 bg-red-100 border border-red-200 hover:text-red-700 hover:bg-red-200 rounded-lg transition" title="Kurangi Stok"><Icon icon="mdi:minus-circle" /></button>
                  <Link :href="`/admin/stock/${p.id}/history`" class="p-1.5 text-blue-500 bg-blue-100 border border-blue-200 hover:text-blue-700 hover:bg-blue-200 rounded-lg transition" title="Riwayat"><Icon icon="mdi:history" /></Link>
                </div>
              </td>
            </tr>
          </tbody>
